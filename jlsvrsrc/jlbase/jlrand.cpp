...
namespace jlsvr
{
    namespace jlbase
    {
        ...
        void CJlRand::SetSeed(uint32_t dwSeedVal)
        {
            init_genrand(dwSeedVal);
            m_dwRandSeed = dwSeedVal;
        }

        uint32_t CJlRand::RandInt(uint32_t min, uint32_t max)
        {
            if (min > max)
            {
        ...
        float CJlRand::RandFloat()
        {
            uint32_t dwRandVal = genrand_int32();
            return (float)(dwRandVal * (1.0 / 4294967295.0f));
        }

        void CJlRand::init_genrand(uint32_t s)
        {
            mt[0] = s & 0xffffffffUL;
        ...
        uint32_t CJlRand::genrand_int32()
        {
            unsigned long y;
